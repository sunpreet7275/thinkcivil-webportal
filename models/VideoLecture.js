...
const videoLectureSchema = new mongoose.Schema({
  ...
  fileLink: {
    ...
    validate: {
      ...
      message: 'Video link is required for videos'
    }
  },
  description: {
    type: String,
...
videoLectureSchema.virtual('children', {
  ref: 'VideoLecture',
  localField: '_id',
  foreignField: 'parent'
});
...
// Indexes
videoLectureSchema.index({ parent: 1 });
videoLectureSchema.index({ createdBy: 1 });
videoLectureSchema.index({ category: 1 });
videoLectureSchema.index({ fullPath: 1, createdBy: 1, category: 1 }, { unique: true });
videoLectureSchema.index({ fileType: 1 });
...
videoLectureSchema.pre('save', function(next) {
  if (this.type === 'file' && !this.fileLink) {
    ...
    err.errors.fileLink = new mongoose.Error.ValidatorError({
      message: 'Video link is required for video files',
      path: 'fileLink',
      value: this.fileLink
    });
    return next(err);
...
videoLectureSchema.statics.deleteRecursive = async function(directoryId) {
...
videoLectureSchema.statics.pathExists = async function(category, fullPath, userId) {
  return await this.findOne({ category, fullPath, createdBy: userId });
};

// Method to get directory tree for a category
videoLectureSchema.statics.getTreeByCategory = async function(category, parentId, userId) {
...
videoLectureSchema.statics.buildTree = function(items) {
  const itemMap = {};
  const rootItems = [];
