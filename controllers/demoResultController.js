...
const getStudentDemoTestResult = async (req, res) => {
  try {
    ...
    const allTestResults = await DemoResultService.getTestResults(req.params.testId);
    const { rank, totalStudents } = calculateRanking(allTestResults, req.user._id);
...
const getStudentDemoResults = async (req, res) => {
  try {
    ...
    for (const testId in resultsByTest) {
      ...
      const allTestResults = await DemoResultService.getTestResults(testId);
...
const getDemoResultById = async (req, res) => {
  try {
    const result = await DemoResultService.getResultById(req.params.id);
    if (!result) {
      return res.status(404).json({ message: 'Demo result not found' });
    }
...
const getStudentDemoResultsByAdmin = async (req, res) => {
  try {
    ...
    const results = await DemoResultService.getStudentDemoResults(studentId);
    ...
    console.error('Get student demo results by admin error:', error);
...
const getStudentDemoTestResultByAdmin = async (req, res) => {
  try {
    ...
    const result = await DemoResultService.getStudentDemoTestResult(testId, studentId);
    if (!result) {
      return res.status(404).json({ 
        message: 'Demo test result not found',
        details: `No result found for testId: ${testId} and studentId: ${studentId}`
      });
    }

    const testResults = await DemoResultService.getTestResults(testId);
    ...
    console.error('Get student demo test result by admin error:', error);
    handleError(res, error, messages.en.serverError);
  }
};
